...
## 2. Example: Transformed data table
...
lapply(vars_factor[1:2], cont_outcome_strat, outcome='cd420')
```

(show graphs and notes)
...
## 5. Example: Selecting relevant predictors and outcome
...
var_predictors <- c(
    ...
    # baseline predictors
    'age', 'wtkg', 'hemo', 'lgbtq', 'hist_intra_drug_use', 'karnof', 'prior_z_30days', #'prior_z', 
...
## 5. Example: Initial data split = 85 % Train / validation + 15% test
...
downsample_rec <- 
  ...
  step_downsample(event, under_ratio = 1.5) %>%
  prep()
...
dat_train_and_val_ds <- upsample_rec %>%
  bake(new_data = NULL) 

dat_train_and_val_ds %>%  count(event)
```

## 5. Example: Validation split = 80% training + 20% validation

```{r dat_val_split}
set.seed(321)
dat_val_split <- validation_split(dat_train_and_val_ds, prop = 0.80, strata = strat_hist_art)
dat_val_split
...
df_dimensions <- list(
  dat_train_and_val_ds_dataset = dat_train_and_val_ds %>% dim,
...
## 6. Example: Random Forest -- k-fold Cross-validation

```{r rf_train_w_cv}
# Generate k folds for CV
num_folds <- 3
dat_folds <- vfold_cv(dat_train_and_val_ds, v = num_folds, strata = strat_hist_art)
...
## 6. Example --- Multiple models: Random forest, Decision Tree, and XGBoost
...
## 6. Example --- XGBoost automated parameter tuning with cross-validation
...
```{r}
# Shut the parallel backend down
...
```{r xgb_tune_metrics, fig.height=4}
...
```{r}
tune_params <- c('learn_rate', 'trees', 'tree_depth', 'mtry')

lapply(tune_params, function(param) {
  xgb_tune_metrics %>%
    filter(.metric == 'roc_auc') %>%
    arrange_at('mean') %>%
...
## 7. Example: Confusion matrix on a single CV fold
...
## 7. Example: Average Confusion matrix over k folds
